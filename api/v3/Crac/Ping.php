...
function _civicrm_api3_crac_ping_spec(&$params)
{
    ...
    $params['access_type']  = array(
        'name'         => 'access_type',
        'api.required' => 0,
        'title'        => 'Access Type',
        'description'  => 'Indicates the type of access, as used for the message to the user. Should be one of "access", "edit" or "use".',
    );
}
...
function civicrm_api3_crac_ping($params)
{
    ...
    $access_type     = CRM_Utils_Array::value('access_type', $params, 'access');
    $exclude_contact = CRM_Utils_Array::value('contact_id', $params, CRM_Core_Session::getLoggedInContactID());
    $users           = $resource->getUsers($exclude_contact, $access_type);
    return civicrm_api3_create_success($users);
}
